...
class GameScene: SKScene { //, SKPhysicsContactDelegate {
    ...
    func move(node: SKNode, to: CGPoint, speed: CGFloat, completion:(() -> Void)? = nil) { // двигает node в заданную точку (to) с заданной скоростью (speed), функция completion не принимает аргументов, опциональна и может быть nil
        // рассчитаем длительность движения на основе скорости
        let x = node.position.x // получим х
        let y = node.position.y // получим y
        let distance = sqrt((x - to.x) * (x - to.x) + (y - to.y) * (y - to.y)) // дистанция по стандартной формуле корень квадратный из квадрата разницы x и y
        let duration = TimeInterval(distance / speed)
        let move = SKAction.move(to: to, duration: duration) // действие движения
        node.run(move, completion: completion ?? { }) // начать перемещение move, по завершению вызвать completion, если его нет то пустой блок
    }
